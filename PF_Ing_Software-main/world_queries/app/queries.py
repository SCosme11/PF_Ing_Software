...
def consulta_poblacion_promedio_continente():
    # consulta #1: la poblacion promedio entre todas las ciudades en la db
    resultados = db.session.query(
    Country.Continent,
    func.avg(Country.Population).label('AveragePopulation')
).group_by(Country.Continent).all()

    return resultados #ya que está devolviendo una tupla y sólo queremos el primer elemento
